<?php
   ...
   session_start();
       ...
       /*BITACORA*/
       $consulta="SELECT CodigoObjeto from tbl_objetos where Objeto = 'editarMesa.php'";
       ...
       $CodObjeto=$numero;
       $accion='Editar';
       $descrip='Se edito una mesa';

  $cod=$_GET['CodigoMesa'];

  $consulta_estados=mysqli_query($conexion,"SELECT Descripcion FROM tbl_estadomesa");

  /*Obtengo valores de la mesa*/
  $consulta="SELECT * from tbl_mesas where CodigoMesa = '$cod'";
  $valor = mysqli_query ($conexion,$consulta);
  $variable= mysqli_fetch_array ($valor);
  $nombre=$variable['Nombre'];
  $codestado=$variable['CodigoEstado'];

  //estado
  $consultestado="SELECT Descripcion from tbl_estadomesa where CodigoEstado = '$codestado'";
  ...
  $descripEstado=$valor[0];
   ...
   if(!empty($_POST)){
    ...
    //estado descripcion
    $consultestado="SELECT CodigoEstado from tbl_estadomesa where Descripcion = '$estado'";
    $consult = mysqli_query ($conexion,$consultestado);
    $valor= mysqli_fetch_array ($consult);
    $codestado=$valor[0];
    ...
    if($nombre==''){
     ...
     //Insert en tabla estado pedido
     $insert=mysqli_query($conexion,"update tbl_mesas set Nombre = '$nombre', CodigoEstado = '$codestado' where CodigoMesa = '$cod'");

     if($insert){
       /*BITACORA*/
       $consulta="SELECT CodigoObjeto from tbl_objetos where Objeto = 'AgregarMesa.php'";
       $consulta1 = mysqli_query($conexion,$consulta);
       $valor= mysqli_fetch_array($consulta1);
       $numero=$valor[0];
       //Llamada a la funcion bitacora
              ...
              <!-- form start -->
                ...
                <div class="card-body">
                  ...
                  <form  action="" method="POST">
                    ...
                    <input value='<?php echo $nombre?>' OnKeyUp="this.value=this.value.toUpperCase();" onkeypress="comprobarEspacios(); return sololetras(event);" id="nombre" name='nombre' type="text" class="form-control" placeholder="INGRESE EL NUMERO O NOMBRE" >
                  </div>
                  <div> 
                  <label for="">ESTADO</label>
                      <select id="estado" name="estado" class="form-control" autofocus>
                        <option selected><?php echo $descripEstado?></option>
                        <?php
                            while($estados=mysqli_fetch_array($consulta_estados))
                            {
                          ?>    
                        <option> <?php echo $estados['Descripcion']?> </option>     
                          <?php 
                            }         
                          ?>   
                      </select>
...
<!-- Page specific script -->
               ...
               <?php } ?>
                  ...
                  <div class="col-md-2">
                  <a href="mesas.php" class="btn btn-danger " role="button" aria-pressed="true">Salir</a>
                  </div>
                  </div>
                  </div> 
                  </div> 
