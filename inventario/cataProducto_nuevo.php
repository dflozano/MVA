...
/*BITACORA*/
          $consulta="SELECT CodigoObjeto from tbl_objetos where Objeto = 'cataProducto_nuevo.php'";
          ...
          $CodObjeto=$numero;
          $accion='Guardar';
          $descrip='Se agrego materia prima';
...
if(!empty($_POST['btnguardar']))
	{
      ...
      $consulta = "SELECT * FROM tbl_catalogomateriaprima WHERE NombreProducto ='$nombre' " ;
      ...
      if ($filas > 0) {
        ...
        $query_insert = mysqli_query($conn,"INSERT INTO tbl_catalogomateriaprima( `NombreProducto`,`Medida`,`StockMinimo`) VALUES('$nombre','$medida','$stock')");

        /*Obtener el ultimo codigoMateria*/
        $ultimo_codigo=mysqli_insert_id($conn);

        $query_insert1 = mysqli_query($conn,"INSERT INTO `tbl_inventarioexistente`(`CodigoMateriaPrima`, `Unidades`) VALUES ('$ultimo_codigo',0)");

        if($query_insert AND $query_insert1){

          echo '<script>
          alert(" Se agrego exitosamente. ");
          window.location="cata_producto.php";
          </script>';
        }else{
        $alert='<p class="msg_error">.</p>';
        echo '<script>
        alert("Error al agregar.");
        window.location="cataProducto_nuevo.php";
        </script>';
        }
      }
   ...
   <div class="col-md-9">
            ...
            <div class="card card-info">
              ...
              <div class="card-header text-center">
                <h3 class="card-title">Ingreso de Materia Prima</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              ...
              <div class="card-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-7">
                      <input onkeypress="comprobarEspacios(); return sololetras(event);" maxlength="50" minlength="10" OnKeyUp="this.value=this.value.toUpperCase();" value="<?php if(isset($nombre)) echo $nombre  ?>" type="text" name="NombreProducto" id="NombreProducto" class="form-control" placeholder="Nombre de la materia prima" >
          </div>
          </div>
                  <div class="form-group row">
                    <label for="" class="col-sm-4 col-form-label">Medida</label>
                    <div class="col-sm-7">
                      <input onkeypress="comprobarEspacios(); return sololetras(event);" maxlength="50" minlength="10" OnKeyUp="this.value=this.value.toUpperCase();" value="<?php if(isset($nombre)) echo $nombre  ?>" type="text" name="Medida" id="Medida" class="form-control" placeholder="Medida de la materia prima" >
          </div>
          
                  </div>
          <div class="form-group row">
            <label for="" class="col-sm-4 col-form-label">Stock mínimo</label>
            <div class="col-sm-3">
             <input onkeypress="return solonumeros(event);" value=1 min="1" id="stock" name="stock" type="number" class="form-control" placeholder="Ingrese" required>
            </div>
          </div>
