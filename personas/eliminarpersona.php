...
$CodObjeto=42;
$accion='Eliminar';
$descrip='Se elimino una persona';
bitacora($CodObjeto,$accion,$descrip);


if(isset($_POST['Aceptar'])) {
  if ('Aceptar'){
    $consulta="DELETE FROM tbl_contacto WHERE CodigoPersona = $cod";
    $conResul=mysqli_query($conexion,$consulta);
  
    $consulta1="DELETE FROM tbl_personas WHERE CodigoPersona = $cod";
    $conResul1=mysqli_query($conexion,$consulta1);
  echo '<script>
    ...
    window.location="tablapersonas.php.php";
    </script>';
  }
}
...
if (isset($_GET['CodigoPersona'])) {
  $cod=$_GET['CodigoPersona'];
  $consulta="SELECT a.CodigoPersona, a.NombreCompleto, 
  a.NumeroIdentidad, c.Telefono, 
  c.Direccion, c.Correo, b.TipoPersona FROM Tbl_Personas as a 
  INNER JOIN Tbl_TipoPersona as b ON a.CodigoTipoPersona = b.CodigoTipoPersona 
  INNER JOIN tbl_contacto as c ON a.CodigoPersona = c.CodigoPersona 
  where a.CodigoPersona='$cod'";
  $conResul=mysqli_query($conexion,$consulta);
  $valores= mysqli_fetch_array ($conResul);
  $nombre=$valores['NombreCompleto'];
  $id=$valores['NumeroIdentidad'];
  $tipo=$valores['TipoPersona'];
...
<section id="content-header">
  ...
  <div class="card card-danger">
              <div class="card-header">
                <h3 class="card-title">¿Está seguro de eliminar <?php echo $nombre; ?>?</h3>
              </div>
             <!-- /.card-header -->
              <!-- form start -->
              ...
              <div class="card-body">
                  ...
                  <p>IDE:<span> <?php echo $id; ?></span></p>
                  <p>Tipo de Persona:<span> <?php echo $tipo; ?></span></p>
